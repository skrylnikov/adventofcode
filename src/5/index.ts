...
//crate Z is on the bottom, and crate N is on top. Stack 2 contains three crates; from bottom to top, they are crates M, C, and D. Finally, stack 3 contains a single crate, P.
...
const [inputMap, inputInstructions] = inputFile.split('\n\n');


const dataMap1: Array<Array<string>> = [];

reverse(inputMap.split('\n')).forEach((line) => {
  for(let i = 0; i < line.length; i+=4) {
    const item = line.substring(i, i+3);
    if(item.includes('[')) {
      if(!dataMap1[i/4]) {
        dataMap1[i/4] = [];
      }

      dataMap1[i/4].push(item.replace('[', '').replace(']', ''));
...
const instructionList = pipe(
  inputInstructions,
  x => x.split('\n'),
  map(line =>{
    const itemList = line.split(' ');
      const move = Number.parseInt(itemList[1]);
      const from = Number.parseInt(itemList[3]);
      const to = Number.parseInt(itemList[5]);
...
const dataMap2: typeof dataMap1 = JSON.parse(JSON.stringify(dataMap1));

instructionList.forEach(instruction => {
  const from = dataMap1[instruction.from-1];
  const list = from.splice(from.length-instruction.move, instruction.move);
  dataMap1[instruction.to-1].push(...reverse(list));
});

const result = dataMap1.map(x => x[x.length-1]).join('');

console.log('result1', result);


instructionList.forEach(instruction => {
  const from = dataMap2[instruction.from-1];
  const list = from.splice(from.length-instruction.move, instruction.move);
  dataMap2[instruction.to-1].push(...list);
});

const result2 = dataMap2.map(x => x[x.length-1]).join('');
